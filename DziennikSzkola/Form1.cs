...
namespace MiniDziennik
{
    public partial class Form1 : Form
    {
        ...
        {
            DodajEdytujOkno oknoDodajEdytuj = new DodajEdytujOkno("Dodaj", new Uczniowie(), sqlContext.Klasa.ToList());

            if(oknoDodajEdytuj.ShowDialog() == DialogResult.OK)
            {
                sqlContext.Uczniowie.Add(new Uczniowie()
                {
                    Imie = oknoDodajEdytuj.DodanyUczen.Imie,
                    Nazwisko = oknoDodajEdytuj.DodanyUczen.Nazwisko,
                    KlasaId = oknoDodajEdytuj.DodanyUczen.KlasaId,
                    RokUrodzenia = oknoDodajEdytuj.DodanyUczen.RokUrodzenia
                }); 
                sqlContext.SaveChanges();
            }
        ...
        {
            UczniowieKlasy wybranyUczen = (tabelaUczniowGridView.CurrentRow.DataBoundItem as UczniowieKlasy);
            Uczniowie edytowanyUczen = sqlContext.Uczniowie.First(x => x.Id == wybranyUczen.Id);

            DodajEdytujOkno oknoDodajEdytuj = new DodajEdytujOkno("Edytuj" , edytowanyUczen, sqlContext.Klasa.ToList());

            if (oknoDodajEdytuj.ShowDialog() == DialogResult.OK)
            {
                
                sqlContext.Update(oknoDodajEdytuj.DodanyUczen);
                edytowanyUczen = oknoDodajEdytuj.DodanyUczen;
                sqlContext.SaveChanges();
            }
        } 
        private void usunButton_Click(object sender, EventArgs e)
        {
            ...
            DialogResult dialogResult = MessageBox.Show("Czy na pewno chcesz usunąć ucznia?", "Usunięcie studenta", MessageBoxButtons.YesNo);
            sqlContext.Uczniowie.Remove(edytowanyUczen);
            sqlContext.SaveChanges();
        }
